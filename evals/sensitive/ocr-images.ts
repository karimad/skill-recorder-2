...
const TESSDATA_BASE = "https://raw.githubusercontent.com/tesseract-ocr/tessdata_fast/main";
...
interface OcrImageCase {
  id: string;
  about: string;
  /** tessdata codes to load, primary first. */
  languages: string[];
  rows: Row[];
...
function poolFor(languages: string[]): Ocr {
  const key = languages.join("+");
  let pool = ocrPools.get(key);
  if (!pool) {
    pool = new Ocr({ langPath: CACHE_DIR, languages, poolSize: 1 });
    ocrPools.set(key, pool);
  }
  return pool;
}
...
async function scoreCase(sharp: SharpModule, c: OcrImageCase, keep: boolean): Promise<CaseResult> {
  for (const code of c.languages) {
    if (!(await ensureTraineddata(code))) {
      return { id: c.id, about: c.about, languages: c.languages, status: "skip", detail: `no traineddata for "${code}" (offline?)` };
    }
  }
  ...
  try {
    words = await poolFor(c.languages).recognize(rendered.buffer);
  } catch (err) {
    return { id: c.id, about: c.about, languages: c.languages, status: "skip", detail: `OCR engine unavailable: ${err instanceof Error ? err.message : err}` };
  }
...
async function environmentUsable(sharp: SharpModule): Promise<boolean> {
  ...
  try {
    ...
    const words = await poolFor(["eng"]).recognize(probe.buffer);
...
async function main(): Promise<void> {
  ...
  for (const r of results) {
    ...
    console.error(`  ${tag}  ${r.id} [${r.languages.join("+")}] — ${r.about}`);
