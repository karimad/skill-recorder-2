...
export interface SensitiveMatch {
   ...
   *  Secrets rank highest, then structured PII, then model-based named entities. */
  rank: number;
}
...
const STRUCTURED_PII: DetectorSpec[] = [
  ...
  },
  {
    ...
    // country code (+1 / 1) may lead it.
    pattern: /(?:\+?1[\s.-]?)?(?:\(\d{3}\)[\s.-]?|\d{3}[\s.-])\d{3}[\s.-]\d{4}(?!\d)/g,
    accept: (value) => {
 ...
 *  every detection layer (secrets, structured PII, named entities). */
 ...
 * and deterministic. Secrets and named entities are detected by the main-process
 * layers and merged separately.
 */
 ...
 */
export function redactedSnippet(text: string, match: SensitiveMatch, pad = 32): string {
  const from = Math.max(0, match.start - pad);
  const to = Math.min(text.length, match.end + pad);
  const slice = text.slice(from, to);
  const local: SensitiveMatch = { ...match, start: match.start - from, end: match.end - from };
...
/** Human label for a finding's category, grouped for the review UI. */
