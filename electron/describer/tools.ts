...
export function createDescriberTools(ctx: ToolContext): Tool[] {
  ...
  const getFrames: Tool = {
    ...
    handler: async (raw) => {
      ...
      if (protect && !redactor!.ready) {
        return {
          textResultForLlm:
            "Advanced protection is on but the on-device OCR model isn't ready yet, so screen frames are being withheld for this window to avoid sending unredacted on-screen text. Proceeding on events and narration; frames become available once the model finishes loading.",
          resultType: "success",
        };
      }
