// Secret/credential detection layer (always on). Wraps `@secretlint/core` — an
 ...
 * engine failure yields no matches (the always-on structured-PII layer still
...
/** Test/util seam: preload the engine (so the first real scan isn't slow). */
export async function warmSecrets(): Promise<boolean> {
